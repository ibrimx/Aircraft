...
export const LayersPanel: FC<LayersPanelProps> = ({
  ...
  const rowStyle = (layer: LayerItem): CSSProperties => ({
    ...
    alignItems: 'center',
    height: 32,
    paddingInlineStart: layer.depth * 16 + parseInt(SPACING[2], 10),
  ...
  return (
    ...
    >
      ...
      <ScrollArea style={{ flex: 1 }}>
        {flat.length === 0 ? (
          <div style={{ display: 'flex', justifyContent: 'center', padding: SPACING[4], color: theme.colors.text.tertiary, fontSize: 13 }}>
            No layers
          </div>
        ) : flat.map((layer) => (
          <div
            key={layer.id}
            style={rowStyle(layer)}
            ...
            onDrop={(e) => {
              ...
              if (srcId !== layer.id) onReorder(srcId, flat.indexOf(layer))
            }}
            onDragOver={(e) => e.preventDefault()}
          >
            <span style={{ width: 16, fontSize: 11, color: theme.colors.text.secondary }}>
              {layer.children?.length ? '\u25B8' : ' '}
            </span>
            <span style={{ fontSize: 11, color: theme.colors.text.secondary, width: 16 }}>
            ...
            {editingId === layer.id ? (
              ...
              <span
                style={{
                  flex: 1,
                  fontSize: 13,
                  color: selectedIds.includes(layer.id)

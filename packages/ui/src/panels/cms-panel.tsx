  ...
  if (sources.length === 0 && !loading) {
    return (
      <div className={className} style={{ background: theme.colors.surface.default, border: `1px solid ${theme.colors.border.subtle}`, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: SPACING[4], gap: SPACING[2], ...style }}>
        <span style={{ fontSize: 13, color: theme.colors.text.tertiary }}>No sources connected</span>
    ...
    <div className={className} style={{ background: theme.colors.surface.default, border: `1px solid ${theme.colors.border.subtle}`, display: 'flex', flexDirection: 'column', ...style }}>
      ...
      <ScrollArea style={{ flex: 1, minBlockSize: 0 }}>
        <div style={{ padding: `0 ${SPACING[3]}px`, display: 'flex', flexDirection: 'column', gap: SPACING[3], paddingBlockEnd: SPACING[3] }}>
          {/* Step 1: Source */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: SPACING[2], paddingBlockStart: SPACING[3] }}>
          ...
          {selectedSourceId && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: SPACING[2] }}>
              ...
              {loading && !selectedCollectionId ? <Skeleton style={{ blockSize: 32, borderRadius: theme.radii.sm }} /> : (
          ...
          {selectedCollection && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: SPACING[2] }}>
              ...
              {loading ? Array.from({ length: 3 }).map((_, i) => <Skeleton key={i} style={{ blockSize: 32, borderRadius: theme.radii.sm }} />) : (
          ...
          {selectedCollection && records.length > 0 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: SPACING[1] }}>
              ...
              {records.slice(0, 5).map((rec) => (
                <div key={rec.id} onClick={() => toggleRecord(rec.id)} style={{ borderRadius: theme.radii.sm, background: theme.colors.surface.raised, cursor: 'pointer', overflow: 'hidden' }}>
                  <div style={{ paddingInline: SPACING[2], paddingBlock: SPACING[1], fontSize: 13, color: theme.colors.text.primary }}>{truncate(rec.title, 50)}</div>
                  {expandedRecords.has(rec.id) && (
                    <div style={{ paddingInline: SPACING[2], paddingBlockEnd: SPACING[2], display: 'flex', flexDirection: 'column', gap: 2 }}>
                      {Object.entries(rec.fields).map(([key, val]) => (
                        <div key={key} style={{ display: 'flex', gap: SPACING[1] }}>
                          <span style={{ fontSize: 11, color: theme.colors.text.secondary, minInlineSize: 60 }}>{key}:</span>
                          <span style={{ fontSize: 11, color: theme.colors.text.tertiary }}>{truncate(String(val ?? ''), 50)}</span>
                        </div>
                      ))}
                    </div>
                  )}

...
import { Surface, Button, Badge, Skeleton } from '@aircraft/ui'
  ...
  return (
    <div className={className} style={wrapStyle}>
      <Surface variant="raised" style={cardStyle}>
        {status === 'validating' && (
          <>
            <div style={{ width: 40, height: 40, borderRadius: '50%', border: `3px solid ${theme.colors.border.default}`, borderTopColor: theme.colors.accent.default, animation: 'spin 800ms linear infinite' }} />
            <span style={{ color: theme.colors.text.secondary, fontSize: 13 }}>Verifying invite…</span>
          </>
        )}
        ...
        {status === 'valid' && (
          <>
            ...
            <h2 style={{ fontSize: 20, fontWeight: 600, color: theme.colors.text.primary, margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '100%' }}>{workspaceName}</h2>
            <p style={{ fontSize: 13, color: theme.colors.text.secondary, margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '100%' }}>{inviterName} invited you as <strong>{role}</strong></p>
            <span style={{ fontSize: 12, color: countdown === 'Expired' ? theme.colors.destructive.default : theme.colors.text.tertiary }}>{countdown}</span>
      ...
      {errorMessage && status !== 'error' && (
        <p style={{ color: theme.colors.destructive.default, fontSize: 12, marginBlockStart: SPACING[2], textAlign: 'center' }}>{errorMessage}</p>
      )}
    </div>
  )
}
